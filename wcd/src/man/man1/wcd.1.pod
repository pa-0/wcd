...
When you are using a very old shell or a non POSIX compliant shell that
doesn't support "$()" command substitution you have to use old style
...
In Unix: C<DIR/.rtdata.wcd>, in DOS: C<DIR\rtdata.wcd>). Do not read
the default treedata file. If DIR is a file, wcd will add DIR instead
of C<DIR/.rtdata.wcd> or C<DIR\rtdata.wcd>. The relative treedata file
...
Wcd opens file C</mnt/network/.rtdata.wcd> The file contains the paths
...
=head2 Install for POSIX shells

For POSIX shell (sh, bash, zsh, ksh, pdksh etc), add the following
function to the shell staartup file (for Bash C<$HOME/.bashrc>):
...
Windows MSYS/ZSH. See section FILES for more information.
...
In a OS/2 console an os2-program cannot change the current work
...
programs, like wcd, don't need to be aware of this and can operate
...
edited with a text-editor. The win32 console version of wcd behaves as
...
The program. In a Bourne-like or C shell the program is always called by a
function or alias, because the current working directory of a Bourne-like
or C shell can only be changed by the builtin cd command. See also section
INSTALLATION.
...
If HOME is set on DOS/Windows, wcd will place all it's files
...
started; when wcd exits, the screen will be restored. One can set this
variable e.g. in C<AUTOEXEC.BAT>. Example:
...
directories that don't match the filter(s) are ignored. A list can be
specified by separating filters with colons (:) on Unix/Cygwin and
semicolons (;) on DOS/Windows systems (Similar as specifying the PATH
variable). Filters are case sensitive on Unix and case insensitive on
DOS/Windows.
...
The paths specified with environment C<WCDBAN> will be banned by wcd.
See also option B<-b>. Specify a list of paths separated by colons on
...
separated by colons on Unix/Cygwin and semicolons on DOS/Windows.
...
directories are. If this variable is not set wcd will assume C</home> on Unix,
