...
than \(gaC\(aq, before you can use a language priority list through the LANGUAGE
...
default value is /usr/local/share/locale. By typing \(gawcd -V\(aq wcd will print
...
Windows console code page is changed with the \(gachcp\(aq command.
...
Wcd has been \(gasoft\(aq converted to Unicode. In its core wcd handles all data
...
The program. Do not rename it to \(gawcd\(aq on Unix systems. In a Bourne-like or
