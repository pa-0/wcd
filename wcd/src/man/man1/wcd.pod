...
not be in legacy mode (check the console's properties). When the console Layout
property "Wrap text output on resize" is disabled you may need to refresh the
screen by pressing the F5 key.
